  ...
  received(data) {
    ...
    indexChats(function (response) {
      var htmlString = response.chats.map(function(chat) {
        ...
        `;
      }).join(''); // Ensure to join the array into a single string

      // Assuming you want to append this HTML to a specific element
      $('#chat-list').html(htmlString); // Replace '#chat-list' with your actual element ID
    });
  }
});